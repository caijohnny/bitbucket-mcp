export function registerRepositoryTools(client) {
    return {
        list_projects: {
        ...
        browse_repository: {
            ...
            handler: async (args) => {
                const result = await client.browseRepository(args.projectKey, args.repoSlug, args.path || '', args.at);
                return {
                    content: [
                        {
                            type: 'text',
        ...
        get_file_content: {
            ...
            handler: async (args) => {
                const content = await client.getFileContent(args.projectKey, args.repoSlug, args.path, args.at);
                return {
                    content: [
                        {
                            type: 'text',
        ...
        search_code: {
            ...
            handler: async (args) => {
                const result = await client.searchCode(args.query, args.projectKey, args.repoSlug, args.limit);
                return {
                    content: [
                        {
                            type: 'text',
                            text: JSON.stringify({
                                ...
                                results: result.code?.values.map((v) => ({
                                    file: v.file.path,
                                    repository: `${v.repository.project.key}/${v.repository.slug}`,
                                    matches: v.hitContexts.map((ctx) => ctx.context.map((c) => ({
                                        line: c.line.line,
                                        text: c.line.text,
