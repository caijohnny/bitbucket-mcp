...
export function registerReviewPrompts(client: BitbucketClient) {
  return {
    review_my_prs: {
      name: 'review_my_prs',
...
function buildReviewPrompt(
  pr: { id: number; version: number; title: string; description?: string; author: { user: { displayName: string } }; fromRef: { displayId: string }; toRef: { displayId: string }; createdDate: number },
  diff: string,
  existingComments: Array<{ id?: number; text?: string; author?: { displayName?: string } } | undefined>,
  projectKey: string,
  repoSlug: string
): string {
  const validComments = existingComments.filter((c): c is { id?: number; text?: string; author?: { displayName?: string } } => c !== undefined);
